...
import requests
from .ni_exceptions import ColourNotInRangeException
...
class DWK:
    ...
    def add_embed(self, embed: Union[NE, Dict[str, Any]]) -> None:
        """
        adds an embedded rich content
        :param embed: embed object or dict
        """
        self.embeds.append(embed.__dict__ if isinstance(embed, NE) else embed)
    ...
    def delete(
        self, sent_webhook: Union[List["DWK"], "DWK"]
    ) -> Union[List[requests.Response], requests.Response]:
        """
        ...
        """
        if not isinstance(sent_webhook, list):
            sent_webhook = cast(List[DWK], [sent_webhook])
        responses: List[requests.Response] = []
        for i, webhook in enumerate(sent_webhook):
            assert isinstance(webhook.content, bytes)
